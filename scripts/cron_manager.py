...
def cron_status() -> dict[str, Any]:
    if not has_crontab():
        return {"ok": True, "installed": False, "available": False, "command": cron_command(), "wechat_text": guidance_text(False)}
    listing = read_crontab()
    installed = any(marker in listing for marker in all_cron_markers())
    return {"ok": True, "installed": installed, "available": True, "command": cron_command(), "wechat_text": guidance_text(installed)}
...
def install_cron(confirm: bool = False) -> dict[str, Any]:
    ...
    if not confirm:
        return {
            ...
            "wechat_text": "我需要在服务器 crontab 增加一条每5分钟唤醒任务。回复“确认开启自动化”后我再安装。",
        }
    if not has_crontab():
        return {"ok": False, "error": "当前环境没有 crontab 命令", "command": cron_command(), "wechat_text": guidance_text(False)}
    listing = read_crontab()
    if any(marker in listing for marker in all_cron_markers()):
        result = {"ok": True, "installed": True, "wechat_text": "自动化唤醒器已经开启。"}
    ...
    result = {"ok": True, "installed": True, "wechat_text": "自动化唤醒器已开启：每5分钟检查一次到期任务。"}
...
def uninstall_cron(confirm: bool = False) -> dict[str, Any]:
    if not confirm:
        return {"ok": False, "requires_confirmation": True, "wechat_text": "确认要停止 lotto-Agent 自动化唤醒器吗？回复“确认停止自动化”后我再执行。"}
    if not has_crontab():
    ...
    return {"ok": True, "installed": False, "wechat_text": "自动化唤醒器已停止，已保存的任务不会被自动执行。"}
